...
module.exports = {
  rules: {
    // Goal: Sort import declarations by various criteria.
    //
    // Requirements:
    ...
    //
    // Roadmap:
    //   - TODO@nw: Should still format when sorted but has whitespace before the import declarations
    'jsort-imports': {
      meta: {
        type: 'suggestion',
        ...
        docs: {
          ...
          recommended: true,
          url:
            'https://github.com/Nate-Wilkins/eslint-plugin-jsort/tree/master/docs/rules/sort-imports',
        },
        ...
        schema: [
          {
            ...
            properties: {
              ...
              // TODO@nw: Would be kindof cool if this worked with `normalizeSourcePaths`.
              ...
              // TODO@nw: Normalize source paths (ie '../someFile' <-> './../someFile')
      ...
      create: function(context) {
          ...
          switch (importDeclaration.declarationType) {
            ...
            case 'value-multiple': {
              ...
              return `import ${
                ...
                config.indent
              }${importMultipleSourceCodeMultiLine}\n} from ${
                importDeclaration.node.source.raw
              };`;
            }
        ...
        function sortByDeclarationTypeGroupIndex(a, b) {
          const aTypeGroupIndex = getDeclarationTypeGroupIndex(
            a.declarationType,
          );
          const bTypeGroupIndex = getDeclarationTypeGroupIndex(
            b.declarationType,
          );
        ...
        function getHumanReadableNode(node) {
          ...
          return {
            ...
            specifiers: node.specifiers.map(s =>
              getHumanReadableSpecifierNode(s),
            ),
          };
        }
          ...
          onCodePathEnd: function() {
            ...
            if (!config.ignoreMemberSort) {
              importDeclarations = importDeclarations.map(importDeclaration => {
                return {
                  ...
                  node: {
                    ...importDeclaration.node,
                    specifiers: importDeclaration.node.specifiers.sort(
                      (a, b) => {
                        let s = 0;
            ...
            if (unsortedNode) {
              context.report({
                ...
                data: {
                  memberName: firstUnsortedImportDeclaration
                ...
                fix(fixer) {
                  ...
                  if (
                    importDeclarations.some(
                      importDeclaration =>
                        importDeclaration.hasCommentsAttached,
                    )
                  ) {
                    return null;
                  }
                  ...
                  return [
                    ...fixes,
                    fixer.insertTextBefore(programNode, lines.join('\n')),
                  ];
                },
              });
            }
